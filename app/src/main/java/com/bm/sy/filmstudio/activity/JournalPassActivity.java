...
/**
 * 创建人: 李俊男
 * 类名称：JournalPassActivity
 * 类描述：日志通过页面  未通过页面
 * 创建时间：2017/2/14 13:06
 */
public class JournalPassActivity extends BaseFragmentActivity {
    private static final String TAG = "【JournalPassActivity】";
    ...
    private int passKey;//1-通过 | 0-未通过

    public static void start(Activity context,int passKey) {
        Intent starter = new Intent(context, JournalPassActivity.class);
        starter.putExtra("passKey",passKey);
        context.startActivity(starter);
    }

    @Override
    protected void initContentView(Bundle savedInstanceState) {
        setContentView(R.layout.ac_journal_pass);
    ...
    protected void initLogic() {
        passKey = getIntent().getIntExtra("passKey",0);

        //假数据
        for(int i=0;i<3;i++){
            ViewBean bean = new ViewBean();
            bean.setPass(passKey);
            mList.add(bean);
        }
...
//                MessageUtil.sendMessage(mHandler, 1212, pics.get(position).getTime());
    ...
    public class PagerScrollAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int position) {
            return PassFragment.newInstance(position, mList);
        }
        @Override
        public int getCount() {
            return mList.size();
